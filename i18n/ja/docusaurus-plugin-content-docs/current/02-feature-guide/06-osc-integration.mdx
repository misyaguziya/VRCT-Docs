...
:::info[v3.5.0での変更]
OSCQuery（VRChatへのポート通知）のセットアップに失敗し続ける場合、VRCTは一定回数で再試行を打ち切り、OSC本体の送受信は継続して動作するようになりました（従来は無限にリトライしていました）。OSCQueryが確立しない環境でも、`OSC IP Address` / `OSC Port` を手動設定すればチャットボックス送信は利用できます。
:::
