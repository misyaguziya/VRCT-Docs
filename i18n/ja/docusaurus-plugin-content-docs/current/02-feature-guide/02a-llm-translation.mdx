...
このガイドでは、LLM（大規模言語モデル）を翻訳エンジンとして利用する際に、なぜプロンプトが必要か、そしてプロンプトや翻訳可能言語をどのようにカスタマイズできるかを説明します。対象APIは OpenAI / OpenAI互換エンドポイント（v3.5.0〜）/ Ollama / LM Studio / Gemini / Plamo / Groq / Open Router です。
...
- ファイル（APIごと）:
  ...
  - OpenAI互換エンドポイント: OpenAIと同じ書式のYAML（フォルダ内のファイル名を確認してください）
